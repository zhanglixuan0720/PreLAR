...
class VAResNetEncoder(BaseEncoder):
    ...
    def __init__(
        ...
        **dummy_kwargs,
        ...
        if va_method == 'concate':
            input_channels = c * 2
        elif va_method in ['diff', 'flow']:
            input_channels = c
        elif va_method == 'catdiff':
            input_channels = c * 3
        else:
            raise ValueError(f"Unknown va_method: {va_method}")
        self._cnn_net = nn.Sequential()
        self._cnn_net.add_module('convin', nn.Conv2d(input_channels, self._cnn_depth, 3, 2, 1))
