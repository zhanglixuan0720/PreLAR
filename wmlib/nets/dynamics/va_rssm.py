...
from ... import core
from ..modules import get_act, NormLayer, get_act_module
from .base import BaseDynamics, State
from ..va_net import action_encoder


class VAEnsembleRSSM(BaseDynamics):
    r"""
    References:
    ...
    def __init__(
        ...
        self._img_in = nn.Sequential(nn.Linear(img_in_dim, self._hidden),
                                    ...
                                    self._act_module())
        self._obs_out = nn.Sequential(nn.Linear(self._deter + embed_dim, self._hidden),
                                      NormLayer(self._norm, self._hidden),
                                      self._act_module())
        if self._discrete:
        ...
        if not self._pretrain:
            self.action_encoder = action_encoder.ActionEncoder(**action_encoder_kwargs)
    ...
    def img_step(
        ...
        x = torch.cat([prev_stoch, self.action_encode(prev_action,prev_stoch,sample)], -1)
        x = self._img_in(x)
    ...
    def action_encode(self, action,prev_stoch=None,sample=True):
        if self._pretrain:
            return action
        if self.action_encoder.type == 'mix':
            action_code =  self.action_encoder(torch.cat([prev_stoch,action],-1),sample)
            return torch.cat([action_code['deter'],action_code['stoch']],-1)
        else:
            return self.action_encoder(torch.cat([prev_stoch,action],-1),sample)[self.action_encoder.type]
