...
class PreLARPretrain(nn.Module):

    def __init__(self, config, obs_space, act_space, step):
        super(PreLARPretrain, self).__init__()
...
class WorldModel(nn.Module):
    ...
    def __init__(self, config, obs_space, step, enable_fp16=False):
        ...
        self.step = step
        self.sort_rank = config.rank_loss == 'sort_rank'
    ...
    def loss(self, data, state=None):
        ...
        rank_loss = self.rank_loss(post, prior, va_action, va_action_state,sort_rank=self.sort_rank)

        likes = {}
        losses = {'kl': kl_loss, 'va_kl': va_kl_loss, 'rank': rank_loss['rank_loss']}
        ...
        metrics['rank_loss'] = rank_loss['rank_loss'].mean().item()
    ...
    # Action-state consistency loss is referred to as rank loss in the code
    def rank_loss(self, post, prior, va_action, va_action_state, img_decoder=None, sort_rank=True):
        # va_action_state 1 -> T -1 (NOT INCLUDE 0)

        kld = torch.distributions.kl_divergence
        shuffled_idx = torch.randperm(va_action.shape[0])
        
        shuffled_va_action = va_action[shuffled_idx]
        post_init = {k: rearrange(v[:,:-1], 'b t ... -> (b t) ...') for k, v in post.items()}
        shuffled_va_action = rearrange(shuffled_va_action[:,1:], 'b t ... -> (b t) 1 ...')
        shuffled_prior = self.rssm.imagine(shuffled_va_action, post_init)
        prior = {k: rearrange(v[:,1:], 'b t ... -> (b t) ...') for k, v in prior.items()}
        shuffled_prior = {k: rearrange(v, 'b 1 ... -> b ...') for k, v in shuffled_prior.items()}
        va_action_state_bacth = {k: rearrange(v, 'b t ... -> (b t) ...') for k, v in va_action_state.items()}
        shuffled_va_action_state_bacth = {k: rearrange(v[shuffled_idx], 'b t ... -> (b t) ...') for k, v in va_action_state.items()}
        if img_decoder is not None:
            prior_dist = img_decoder(self.rssm.get_feat(prior))
            shuffled_prior_dist = img_decoder(self.rssm.get_feat(shuffled_prior))
        else:
            prior_dist = self.rssm.get_dist(prior)
            shuffled_prior_dist = self.rssm.get_dist(shuffled_prior)
        prior_kl = kld(prior_dist,shuffled_prior_dist)
        prior_kl = prior_kl.detach() # detach
    
        va_action_dist = self.va_encoder.get_dist(va_action_state_bacth)
        shuffled_va_action_dist = self.va_encoder.get_dist(shuffled_va_action_state_bacth)
        va_action_kl = kld(va_action_dist, shuffled_va_action_dist)

        # rank loss
        if sort_rank:
            # numerically unstable
            raise NotImplementedError
            piror_rank = torchsort.soft_rank(prior_kl.unsqueeze(0),regularization_strength=0.001).squeeze(0)
            va_action_rank = torchsort.soft_rank(va_action_kl.unsqueeze(0), regularization_strength=0.01).squeeze(0)
            # pearson correlation coefficient
            pcc = self.pearson_correlation_coefficient(piror_rank,va_action_rank)
            free = torch.tensor(0, device=pcc.device, dtype=pcc.dtype)
            loss = torch.maximum(pcc, free)
        else:
            prior_kl_rdiff = prior_kl[:,None] - prior_kl
            va_action_kl_rdiff = va_action_kl[:,None] - va_action_kl
            prior_kl_rdiff_sign = torch.sign(prior_kl_rdiff)
            va_action_kl_rdiff_sign = torch.sign(va_action_kl_rdiff)
            loss = F.mse_loss(prior_kl_rdiff_sign,va_action_kl_rdiff_sign)

        return {'rank_loss':loss}
