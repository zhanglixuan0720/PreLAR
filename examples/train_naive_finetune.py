...
def main():
    ...
    configs = yaml.load(
        (Path(sys.argv[0]).parent.parent / "configs" / "naive_finetuning.yaml").read_text()
    )
    ...
    # wandb_config['name']= wandb_config['name'] + '-seed' + str(config.seed)
    task_name =  '-'.join(config.task.lower().split("_", 1))

    wandb_config['name']= f'{wandb_config["name"]}-{prtrain_dataset_prefix}-{task_name}-seed{config.seed}'
    outputs = [
    ...
    agent = agents.Naive_Finetune(config, train_envs[0].obs_space, train_envs[0].act_space, step)
